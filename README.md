...
| `code` | ✅ | 股票代码，如 `600887` |
| `--model` | ✅ | 行业估值模型，见下表 |
| `--pe MIN MAX` | | 手动PE区间（单次逃生阀：仅当次报告走线性映射，不做持久化恢复；不传则自动分位评分） |
| `--pb MIN MAX` | | 手动PB区间（单次逃生阀，语义同上） |
...
盈利发生过跃迁的公司（如紫金矿业 5 年净利 20 倍），跃迁前的低 EPS 制造极端历史 PE，拉宽全历史分位区间，评分失真。引擎用年报净利润序列自动检测向上换挡（按首年对齐每 5 年一段取中位数，相邻段比值 >3），检测到时：

- PE 分位只用换挡生效后的子序列，窗口起点对齐年报披露时点（T 年年报次年 5 月起）
- PB 与其余因子维持全历史（净资产是存量，PB 锚仍可用）；窗口内 PE 样本 <50 天时 pe 因子计中性 50 分
- 向下回落、亏损段、历史不足：不切窗，meta.window_reason 记录原因，亏损段在报告中有醒目警示
- 检测在评分引擎外部完成、经参数传入，回测不传窗口参数，PIT 口径不受影响（已逐点回归验证）
